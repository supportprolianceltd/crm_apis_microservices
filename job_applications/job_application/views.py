...
class ResumeScreeningView(APIView):
    ...
    def post(self, request, job_requisition_id):
        try:
            ...
            return Response({
                "detail": "Resume screening started in background. You will be notified when complete.",
                "job_requisition_id": job_requisition_id
            }, status=status.HTTP_202_ACCEPTED)

        except Exception as e:
            logger.exception(f"Error starting background screening for JobRequisition {job_requisition_id}: {str(e)}")
            return Response({
                "detail": f"Failed to start background screening: {str(e)}",
                "document_type": request.data.get('document_type'),
