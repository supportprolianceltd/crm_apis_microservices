...
class JobApplicationListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        logger.info(f"request.user: {request.user}, is_authenticated: {getattr(request.user, 'is_authenticated', None)}")

        # ✅ Extract tenant_id from JWT
        tenant_id = get_tenant_id_from_jwt(self.context['request'])
        if not tenant_id:
            return
            logger.warning("Tenant ID not found in token")
        ...
        # Optional: limit to user branch if available (and not overridden by query param)
