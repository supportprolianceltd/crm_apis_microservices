    ...
    def _download_resume_safe(self, app, applications_data, document_type_lower):
        ...
        try:
            ...
            if not file_info:
                ...
                logger.warning(f"🔍 DOWNLOAD FAILED - No {document_type_lower} for App {app.id}: {download_time:.2f}s")
                return {
                    "app": app,
                    "success": False,
            ...
            if response.status_code != 200:
                download_end = time.perf_counter()
                download_time = download_end - download_start
                logger.warning(f"🔍 DOWNLOAD FAILED - HTTP {response.status_code} for App {app.id}: {download_time:.2f}s")
                return {
                    "app": app,
                    "success": False,
            ...
            if file_content is None:
                download_end = time.perf_counter()
                download_time = download_end - download_start
                logger.warning(f"🔍 DOWNLOAD FAILED - Decompression error for App {app.id}: {download_time:.2f}s")
                return {
                    "app": app,
                    "success": False,
            ...
            download_end = time.perf_counter()
            download_time = download_end - download_start
            logger.info(f"📥 DOWNLOAD SUCCESS - App {app.id}: {download_time:.2f}s")
            ...
            download_end = time.perf_counter()
            download_time = download_end - download_start
            logger.warning(f"⏰ DOWNLOAD TIMEOUT - App {app.id}: {download_time:.2f}s")
            return {
                "app": app,
                "success": False,
                "error": "Download timeout",
            ...
            download_end = time.perf_counter()
            download_time = download_end - download_start
            logger.warning(f"🌐 CONNECTION ERROR - App {app.id}: {download_time:.2f}s")
            return {
                "app": app,
                "success": False,
            ...
            download_end = time.perf_counter()
            download_time = download_end - download_start
            logger.warning(f"❌ DOWNLOAD ERROR - App {app.id} ({str(e)[:50]}...): {download_time:.2f}s")
        ...
        try:
            if not result["success"]:
                ...
                with transaction.atomic():
                    app.screening_status = [{'status': 'failed', 'score': 0.0, 'updated_at': now, 'error': result["error"]}]
                ...
                logger.warning(f"💥 VETTING FAILED - App {app.id}: TOTAL {total_time:.2f}s (DL: {download_time:.2f}s, PROC: {processing_time:.2f}s)")
                return {
                    "application_id": str(app.id),
                    "full_name": app.full_name,
                    "email": app.email,
            ...
            if not resume_text:
                now = timezone.now().isoformat()
                with transaction.atomic():
                    app.screening_status = [{'status': 'failed', 'score': 0.0, 'updated_at': now, 'error': 'Failed to parse resume'}]
                    app.screening_score = 0.0
                    app.save(update_fields=['screening_status', 'screening_score'])
                processing_end = time.perf_counter()
                processing_time = processing_end - processing_start
                ...
                logger.warning(f"📄 PARSE FAILED - App {app.id}: TOTAL {total_time:.2f}s (DL: {download_time:.2f}s, PROC: {processing_time:.2f}s)")
                return {
                    "application_id": str(app.id),
                    "full_name": app.full_name,
                    "email": app.email,
            ...
            logger.info(f"✅ VETTING SUCCESS - App {app.id} (Score: {score:.1f}): TOTAL {total_time:.2f}s (DL: {download_time:.2f}s, PROC: {processing_time:.2f}s)")
            ...
            logger.warning(f"⚠️  VETTING ERROR - App {app.id} ({str(e)[:50]}...): TOTAL {total_time:.2f}s (DL: {download_time:.2f}s, PROC: {processing_time:.2f}s)")
