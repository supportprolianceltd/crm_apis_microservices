...
#     # New field: Tracks history of status changes (automated or manual)
#     status_history = models.JSONField(default=list, blank=True)  # e.g., [{"status": "shortlisted", "changed_by": {"system": "resume_screening"}, "automated": true, "timestamp": "...", "reason": "..."}]
...
#     def append_status_history(self, new_status, automated=True, changed_by=None, reason='Status update'):
#         """
#         Append a new entry to status_history if status is changing.
#         Returns True if appended (and saved), False otherwise.
#         """
...
#             # Ensure status_history is a list
...
#             # Log for auditing
#             logger.info(f"Appended status history for app {self.id}: {new_status} (automated: {automated}, reason: {reason})")
#             return True
#         else:
...
class JobApplication(models.Model):
    ...
    tenant_id = models.CharField(max_length=36, blank=False, null=False)
    job_requisition_id = models.CharField(max_length=36, blank=False, null=False)
    job_requisition_title = models.CharField(max_length=255, blank=False, null=False)  # New field to store the job requisition title
    branch_id = models.CharField(max_length=36, null=True, blank=True)
    ...
    def __str__(self):
        return f"{self.email} {self.full_name} - {self.job_requisition_title} ({self.tenant_id})"
...
class Schedule(models.Model):
    ...
    def __str__(self):
        return f"Schedule for {self.job_application_id} - {self.job_requisition_title} ({self.interview_start_date_time})"
