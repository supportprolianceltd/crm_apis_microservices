    ...
    def get(self, request, course_id=None):
        try:
            try:
                ...
                logger.info(
                    f"Retrieved certificate template for course {course_id}")
                ...
                logger.info(
                    f"No template found for course {course_id}, returning default")
                return Response(default_data)
        except Exception as e:
            logger.error(
    f"Error fetching certificate template: {str(e)}",
     exc_info=True)
            return Response({"detail": "Error fetching certificate template"},
            ...
            if serializer.is_valid():
                ...
                if logo_file:
                    ...
                    if isinstance(logo_url, str) and logo_url:
                        ...
                        logger.warning(f"Invalid logo_url: {logo_url}")
                        instance.logo = None
                ...
                if signature_file:
                    ...
                    if isinstance(signature_url, str) and signature_url:
                        ...
                        logger.warning(
                            f"Invalid signature_url: {signature_url}")
                        instance.signature = None

                serializer = CertificateTemplateSerializer(instance)
                logger.info(
                    f"Updated/Created certificate template for course {course_id}")
                return Response(
                    serializer.data, status=status.HTTP_200_OK if template else status.HTTP_201_CREATED)
            logger.warning(
                f"Invalid data for certificate template: {serializer.errors}")
            return Response(serializer.errors,
                            status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            logger.error(
    f"Error updating certificate template: {str(e)}",
     exc_info=True)
            return Response({"detail": "Error updating certificate template"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class CategoryViewSet(TenantBaseView, viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        # The tenant info will be populated from JWT in serializer's validate()
        ...
        with transaction.atomic():
            ...
            if hasattr(request, 'jwt_payload'):
                instance.created_by = request.jwt_payload
                instance.last_edited_by = request.jwt_payload
                instance.save()
            logger.info(f"Category created: {serializer.validated_data['name']}")
    ...
    partial=kwargs.get(
        ...
        try:
            ...
            logger.error(f"Category update validation failed: {str(e)}")
            raise
        with transaction.atomic():
            serializer.save()
            if hasattr(request, 'jwt_payload'):
    ...
    # REMOVE the get_permissions() method entirely!
...
class CourseViewSet(TenantBaseView, viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        # Add tenant_id and tenant_name from token/context
        data = request.data.copy()
        data['tenant_id'] = request.tenant.get('id') if isinstance(request.tenant, dict) else request.tenant.id
        if hasattr(request, 'jwt_payload'):
            data['tenant_name'] = request.jwt_payload.get('tenant_name')
        serializer = self.get_serializer(data=data)
        try:
            serializer.is_valid(raise_exception=True)
        except ValidationError as e:
            logger.error(f"Course creation validation failed: {str(e)}")
            raise
        with transaction.atomic():
            instance = serializer.save()
            if hasattr(request, 'jwt_payload'):
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.error(f"Course update validation failed: {str(e)}")
            raise
        with transaction.atomic():
            serializer.save()
            if hasattr(request, 'jwt_payload'):
                instance.last_edited_by = request.jwt_payload
                instance.save()
            logger.info(f"Course updated: {instance.title}")
        return Response(serializer.data)
    ...
    instance.thumbnail.name)  # <-- FIX: use .name
            instance.delete()
            logger.info(f"Course deleted: {instance.title}")
        return Response(status=status.HTTP_204_NO_CONTENT)
    ...
    'enrollment', distinct=True)  # <-- FIXED
            ).filter(enrollment_count__gt=0).order_by('-enrollment_count').first()
            if not course:
                logger.info(
                    "No courses with enrollments found for most_popular")
                return Response(
    ...
    'enrollment', distinct=True)  # <-- FIXED
            ...
            logger.info(f"Most popular course: {course.title}")
            return Response(response_data, status=status.HTTP_200_OK)
        except Exception as e:
            logger.error(
    f"Error fetching most popular course: {str(e)}",
     exc_info=True)
            return Response(
    ...
    'enrollment', distinct=True)  # <-- FIXED
            ...
            if not course:
                logger.info(
                    "No courses with enrollments found for least_popular")
                return Response(
                    {"message": "No courses with enrollments yet"},
                    status=status.HTTP_200_OK
                )
    ...
    'enrollment', distinct=True)  # <-- FIXED
            ...
            logger.info(f"Least popular course: {course.title}")
            return Response(response_data, status=status.HTTP_200_OK)
        except Exception as e:
            logger.error(
    f"Error fetching least popular course: {str(e)}",
     exc_info=True)
            return Response(
    ...
    def list(self, request, *args, **kwargs):
        ...
        logger.info(
            f"Listed courses with {response.data['total_all_enrollments']} total enrollments")
        return response

    def get_permissions(self):
        return [IsAdminUser()] if self.action in ['create', 'update', 'partial_update', 'destroy',
                            'assign_instructor', 'update_instructor_assignment', 'remove_instructor'] else [IsAuthenticated()]
    ...
    def assign_instructor(self, request, pk=None):
        try:
            ...
            logger.info(f"Instructor assigned to course {course.title}")
            return Response(CourseInstructorSerializer(
                course_instructor).data, status=status.HTTP_201_CREATED)
        except ValidationError as e:
            logger.error(f"Instructor assignment validation failed: {str(e)}")
            raise
        except Exception as e:
            logger.error(
    f"Error assigning instructor: {str(e)}",
     exc_info=True)
            return Response({"detail": "Error assigning instructor"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)

    @action(detail=True, methods=['put'],
            url_path='instructors/(?P<instructor_id>[^/.]+)')
    def update_instructor_assignment(
        self, request, pk=None, instructor_id=None):
        try:
            course = self.get_object()
            ...
            course_instructor = serializer.save()
            logger.info(
                f"Instructor assignment updated for course {course.title}")
            return Response(CourseInstructorSerializer(course_instructor).data)
        except ValidationError as e:
            logger.error(
                f"Instructor assignment update validation failed: {str(e)}")
            raise
        except Http404:
            logger.warning(
                f"Instructor assignment not found for course {pk} and instructor {instructor_id}")
            return Response(
                {"detail": "Instructor assignment not found"}, status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            logger.error(
    f"Error updating instructor assignment: {str(e)}",
     exc_info=True)
            return Response({"detail": "Error updating instructor assignment"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def remove_instructor(self, request, pk=None, instructor_id=None):
        try:
            ...
            logger.info(
                f"Instructor {instructor_id} removed from course {course.title}")
            return Response(status=status.HTTP_204_NO_CONTENT)
        except Http404:
            logger.warning(
                f"Instructor {instructor_id} not found for course {pk}")
            return Response(
                {"detail": "Instructor assignment not found"}, status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            logger.error(f"Error removing instructor: {str(e)}", exc_info=True)
            return Response({"detail": "Error removing instructor"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class ModuleViewSet(TenantBaseView, viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.error(f"Module creation validation failed: {str(e)}")
            raise
        with transaction.atomic():
            course = get_object_or_404(
    Course, id=course_id) if course_id else None
            ...
            if hasattr(request, 'jwt_payload'):
                ...
                instance.save()
            logger.info(
                f"Module created: {serializer.validated_data['title']}")
        return Response(serializer.data, status=status.HTTP_201_CREATED)
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.error(f"Module update validation failed: {str(e)}")
            raise
        serializer.save()
        if hasattr(request, 'jwt_payload'):
            instance.last_edited_by = request.jwt_payload
            instance.save()
        logger.info(f"Module updated: {instance.title}")
        return Response(serializer.data)
    ...
    def destroy(self, request, *args, **kwargs):
        with transaction.atomic():
            ...
            instance.delete()
            logger.info(f"Module deleted: {instance.title}")
        return Response(status=status.HTTP_204_NO_CONTENT)
    ...
    def bulk_update(self, request, *args, **kwargs):
        ...
        if not isinstance(module_ids, list) or not isinstance(
            is_published, bool):
            logger.warning("Invalid input for bulk_update")
            return Response({"detail": "Invalid input: ids must be a list and is_published must be a boolean"},
                            status=status.HTTP_400_BAD_REQUEST)
        with transaction.atomic():
            updated = Module.objects.filter(
    id__in=module_ids).update(
        is_published=is_published)
            logger.info(f"Bulk updated {updated} modules")
            return Response(
                {"detail": f"Updated {updated} module(s)"}, status=status.HTTP_200_OK)

    @action(detail=False, methods=['post'])
    def bulk_delete(self, request, *args, **kwargs):
        module_ids = request.data.get('ids', [])
        if not isinstance(module_ids, list):
            logger.warning("Invalid input for bulk_delete")
            return Response({"detail": "Invalid input: ids must be a list"},
                            status=status.HTTP_400_BAD_REQUEST)
        with transaction.atomic():
            # Delete associated lesson files
            storage_service = get_storage_service()
            ...
            logger.info(f"Bulk deleted {deleted} modules")
            return Response({"detail": f"Deleted {deleted} module(s)"},
...
class LessonViewSet(TenantBaseView, viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        serializer = self.get_serializer(
            data=request.data,
            context={'course_id': course_id, 'module_id': module_id}
        )
        try:
            serializer.is_valid(raise_exception=True)
        except ValidationError as e:
    ...
    Module, id=module_id, course_id=course_id)
...
class EnrollmentViewSet(TenantBaseView, viewsets.ViewSet):
    ...
    def list(self, request, course_id=None, user_id=None):
        try:
            queryset = Enrollment.objects.select_related(
                'user', 'course').filter(is_active=True)
            if request.user.role != "admin" and user_id and user_id != str(
                request.user.id):
            ...
            if course_id:
                ...
                if not queryset.exists() and not request.user.is_staff:
                    return Response(
                        {"detail": "Not enrolled in this course"}, status=status.HTTP_403_FORBIDDEN)
            ...
            return Response({"detail": "Error fetching enrollments"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)

    @action(detail=False, methods=['post'],
            url_path='course/(?P<course_id>[^/.]+)')
    def enroll_to_course(self, request, course_id=None):
        try:
            course = get_object_or_404(
    Course, id=course_id, status='Published')
            user_id = request.data.get('user_id')
            if not user_id:
                return Response({"detail": "user_id is required"},
                                status=status.HTTP_400_BAD_REQUEST)
            if Enrollment.objects.filter(
                user_id=user_id, course=course).exists():
                return Response(
                    {"detail": "User already enrolled in this course"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response(
                {"detail": "Course not found or not published"}, status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            return Response({"detail": "Error processing enrollment"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)

    @action(detail=False, methods=['post'],
            url_path='course/(?P<course_id>[^/.]+)/bulk')
    def bulk_enroll(self, request, course_id=None):
        tenant = request.tenant
        try:

            # Remove status='Published' for admin flexibility
            course = get_object_or_404(Course, id=course_id)
            user_ids = request.data.get('user_ids', [])
            if not isinstance(user_ids, list):
                logger.warning(
                    f"[{tenant.schema_name}] Invalid user_ids for bulk enrollment")
                return Response({"detail": "user_ids must be a list"},
                                status=status.HTTP_400_BAD_REQUEST)
            if not user_ids:
                logger.warning(
                    f"[{tenant.schema_name}] No user_ids provided for bulk enrollment")
                return Response({"detail": "user_ids is required"},
                                status=status.HTTP_400_BAD_REQUEST)
            existing = set(
    ...
    Enrollment(
        user_id=user_id,
         course=course) for user_id in user_ids if user_id not in existing]
            with transaction.atomic():
                if new_enrollments:
                    Enrollment.objects.bulk_create(new_enrollments)
                logger.info(
                    f"[{tenant.schema_name}] Bulk enrolled {len(new_enrollments)} users to course {course_id}")
                return Response({
            ...
            return Response(
                {"detail": "No new enrollments created (all users already enrolled)"}, status=status.HTTP_200_OK)
        except Http404:
            logger.warning(
                f"[{tenant.schema_name}] Course {course_id} not found")
            return Response({"detail": "Course not found"},
                            status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            logger.error(
    f"[{tenant.schema_name}] Error in bulk enrollment: {str(e)}",
     exc_info=True)
            return Response({"detail": "Error processing bulk enrollment"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    'user', 'course').filter(
        is_active=True).order_by('-enrolled_at')
            paginator = self.pagination_class()
            page = paginator.paginate_queryset(enrollments, request)
            serializer = EnrollmentSerializer(page, many=True)
            return paginator.get_paginated_response(serializer.data)
        except Exception as e:
            return Response({"detail": "Error fetching all enrollments"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def user_enrollments(self, request, user_id=None):
        ...
        try:
            ...
            if request.user.role != "admin" and user_id != str(
                request.user.id):
                return Response({"detail": "Unauthorized access"},
                                status=status.HTTP_403_FORBIDDEN)
            ...
            for enrollment in enrollments:
                ...
                instructors = [
                    {'id': ci.instructor.id,
    'name': ci.instructor.user.get_full_name(),
     'bio': ci.instructor.bio}
            ...
            return Response({"detail": "Error fetching user enrollments"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    user=request.user,
     is_active=True).select_related('course').order_by('-enrolled_at')
            result = []
            for enrollment in enrollments:
                course = enrollment.course
    ...
    total_lessons) *
     100) if total_lessons else 0
            ...
            return Response({"detail": "Error fetching my courses"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)

    @action(detail=False, methods=['get', 'post'],
            url_path='progress')  # Add 'post' here
    def course_progress(self, request):
        ...
        user_id = request.query_params.get(
            'user') or request.data.get('user') or request.user.id
        course_id = request.query_params.get(
            'course') or request.data.get('course')
        ...
        progress = int((completed_lessons / total_lessons)
                       * 100) if total_lessons else 0
    ...
    def update_course_progress(self, request):
        ...
        enrollment = Enrollment.objects.filter(
    user_id=user_id, course_id=course_id, is_active=True).first()
        if not enrollment:
        ...
        total_lessons = Lesson.objects.filter(module__course=course).count()
        completed_lessons = LessonProgress.objects.filter(
    user_id=user_id, lesson__module__course=course, is_completed=True).count()
        progress = int((completed_lessons / total_lessons)
                       * 100) if total_lessons else 0
        return Response({'progress': progress})
    ...
    def complete_lesson(self, request):
        ...
        if not user_id or not lesson_id:
            return Response({'detail': 'Missing user or lesson ID'},
                            status=status.HTTP_400_BAD_REQUEST)
...
class AssignmentViewSet(TenantBaseView, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = Assignment.objects.all() if user.is_staff or hasattr(user, 'instructor_profile') else Assignment.objects.filter(
            course_id__in=Enrollment.objects.filter(
    user=user, is_active=True).values_list(
        'course_id', flat=True)
        )
...
class AssignmentSubmissionViewSet(TenantBaseView, viewsets.ModelViewSet):
    ...
    assignment_id=assignment_id, student=user).exists()
        if exists:
            return Response(
...
class AdminEnrollmentView(TenantBaseView, APIView):
    ...
    data=request.data, many=isinstance(
        request.data, list))
        try:
            serializer.is_valid(raise_exception=True)
        except ValidationError as e:
            logger.error(
                f"[{tenant.schema_name}] Admin enrollment validation failed: {str(e)}")
            raise
        with transaction.atomic():
            enrollments = []
            for data in serializer.validated_data:
                course = get_object_or_404(
    Course, id=data['course_id'], status='Published')
                if not Enrollment.objects.filter(
                    user_id=data['user_id'], course=course).exists():
                    enrollments.append(
    Enrollment(
        user_id=data['user_id'],
          course=course))
            ...
            logger.info(
                f"[{tenant.schema_name}] Created {len(enrollments)} enrollments")
            return Response(
                {"detail": f"{len(enrollments)} enrollments created successfully"}, status=status.HTTP_201_CREATED)
...
class LearningPathViewSet(TenantBaseView, viewsets.ModelViewSet):
    ...
    data=data, context={
        ...
        try:
            ...
            logger.error(
                f"[{tenant.schema_name}] Learning path creation validation failed: {str(e)}")
            raise
        with transaction.atomic():
            instance = serializer.save()
            if hasattr(request, 'jwt_payload'):
                ...
                instance.save()
            logger.info(
                f"[{tenant.schema_name}] Learning path created: {serializer.validated_data['title']}")
            return Response(serializer.data, status=status.HTTP_201_CREATED)
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            instance.delete()
            logger.info(
                f"[{tenant.schema_name}] Learning path deleted: {instance.title}")
        return Response(status=status.HTTP_204_NO_CONTENT)

    def get_permissions(self):
        return [IsAdminUser()] if self.action in ['create', 'update',
                            'partial_update', 'destroy'] else [permissions.AllowAny()]
...
class ResourceViewSet(TenantBaseView, viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.error(
                f"[{tenant.schema_name}] Resource creation validation failed: {str(e)}")
            raise
        with transaction.atomic():
            course = get_object_or_404(
    Course, id=course_id) if course_id else None
            ...
            if hasattr(request, 'jwt_payload'):
                ...
                instance.save()
            logger.info(
                f"[{tenant.schema_name}] Resource created: {serializer.validated_data['title']}")
            return Response(serializer.data, status=status.HTTP_201_CREATED)
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.error(
                f"[{tenant.schema_name}] Resource update validation failed: {str(e)}")
            raise
        serializer.save()
        if hasattr(request, 'jwt_payload'):
            instance.last_edited_by = request.jwt_payload
            instance.save()
        logger.info(
            f"[{tenant.schema_name}] Resource updated: {instance.title}")
        return Response(serializer.data)
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        instance.delete()
        logger.info(
            f"[{tenant.schema_name}] Resource deleted: {instance.title}")
        return Response(status=status.HTTP_204_NO_CONTENT)
    ...
    def reorder(self, request, course_id):
        ...
        if not isinstance(resources, list):
            logger.warning(
                f"[{tenant.schema_name}] Invalid input for resource reorder")
            return Response({"detail": "resources must be a list"},
                            status=status.HTTP_400_BAD_REQUEST)
        try:
            with transaction.atomic():
                for item in resources:
                    Resource.objects.filter(
    id=item['id'], course_id=course_id).update(
        order=item['order'])
                logger.info(
                    f"[{tenant.schema_name}] Reordered resources for course {course_id}")
                return Response(
                    {"detail": "Resources reordered successfully"}, status=status.HTTP_200_OK)
        except Exception as e:
            logger.error(
    f"[{tenant.schema_name}] Error reordering resources: {str(e)}",
     exc_info=True)
            return Response({"detail": "Error reordering resources"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)

    def get_permissions(self):
        return [IsAdminUser()] if self.action in ['create', 'update',
                            'partial_update', 'destroy', 'reorder'] else [IsAuthenticated()]
...
class CertificateView(TenantAPIView, APIView):
    ...
    def get(self, request, course_id=None):
        ...
        try:
            ...
            enrollments = Enrollment.objects.filter(
    user=request.user, is_active=True, completed_at__isnull=False)
            if course_id:
                enrollments = enrollments.filter(course_id=course_id)
            certificates = Certificate.objects.filter(
    enrollment__in=enrollments).select_related('enrollment__course')
            serializer = CertificateSerializer(
    certificates, many=True, context={
        'tenant': tenant})
            logger.info(
                f"[{tenant.schema_name}] Retrieved certificates for user {request.user.id}")
            return Response(serializer.data)
        except Exception as e:
            logger.error(
    f"[{tenant.schema_name}] Error fetching certificates: {str(e)}",
     exc_info=True)
            return Response({"detail": "Error fetching certificates"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class FAQStatsView(APIView):
    ...
    course_id=course_id, is_active=True).count()
            ...
            logger.info(
                f"[{tenant.schema_name}] Retrieved FAQ count: {faq_count}")
            return Response({'faq_count': faq_count})
        except Exception as e:
            logger.error(
    f"[{tenant.schema_name}] Error fetching FAQ stats: {str(e)}",
     exc_info=True)
            return Response({"detail": "Error fetching FAQ stats"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class BadgeViewSet(TenantBaseView, viewsets.ModelViewSet):
    ...
    data=request.data, context={
        ...
        try:
            ...
            logger.error(
                f"[{tenant.schema_name}] Badge creation validation failed: {str(e)}")
            raise
        with transaction.atomic():
            serializer.save()
            logger.info(
                f"[{tenant.schema_name}] Badge created: {serializer.validated_data['title']}")
            return Response(serializer.data, status=status.HTTP_201_CREATED)
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.error(
                f"[{tenant.schema_name}] Badge update validation failed: {str(e)}")
            raise
        serializer.save()
        logger.info(
            f"[{tenant.schema_name}] Badge updated: {instance.title}")
        return Response(serializer.data)
...
class UserPointsViewSet(TenantBaseView, viewsets.ModelViewSet):
    ...
    def leaderboard(self, request):
        ...
        try:
            ...
            page = paginator.paginate_queryset(queryset, request)
            logger.info(
                f"[{tenant.schema_name}] Retrieved leaderboard for course {course_id or 'all'}")
            return paginator.get_paginated_response(page)
        except Exception as e:
            logger.error(
    f"[{tenant.schema_name}] Error fetching leaderboard: {str(e)}",
     exc_info=True)
            return Response(
                {"detail": "Error fetching leaderboard"}, status=500)
...
class UserBadgeViewSet(TenantBaseView, viewsets.ModelViewSet):
    ...
    data=request.data, context={
        ...
        try:
            ...
            logger.error(
                f"[{tenant.schema_name}] User badge creation validation failed: {str(e)}")
            raise
        with transaction.atomic():
            serializer.save()
            logger.info(
                f"[{tenant.schema_name}] User badge created for user {serializer.validated_data['user'].id}")
            return Response(serializer.data, status=status.HTTP_201_CREATED)
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.error(
                f"[{tenant.schema_name}] User badge update validation failed: {str(e)}")
            raise
        serializer.save()
        logger.info(
            f"[{tenant.schema_name}] User badge updated for user {instance.user.id}")
        return Response(serializer.data)
...
class FAQViewSet(TenantBaseView, viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        serializer = self.get_serializer(
    data=request.data, context={
        'tenant': tenant, 'course_id': course_id})
        try:
            serializer.is_valid(raise_exception=True)
        except ValidationError as e:
            logger.error(
                f"[{tenant.schema_name}] FAQ creation validation failed: {str(e)}")
            raise
        with transaction.atomic():
            course = get_object_or_404(
Course, id=course_id) if course_id else None
            ...
            logger.info(
                f"[{tenant.schema_name}] FAQ created for course {course_id}")
            return Response(serializer.data, status=status.HTTP_201_CREATED)
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.error(
                f"[{tenant.schema_name}] FAQ update validation failed: {str(e)}")
            raise
        serializer.save()
        logger.info(
            f"[{tenant.schema_name}] FAQ updated for course {instance.course.id}")
        return Response(serializer.data)
    ...
    def reorder(self, request, course_id):
        ...
        if not isinstance(faqs, list):
            logger.warning(
                f"[{tenant.schema_name}] Invalid input for FAQ reorder")
            return Response({"detail": "faqs must be a list"},
                            status=status.HTTP_400_BAD_REQUEST)
        try:
            with transaction.atomic():
                for item in faqs:
...
course_id=course_id).update(
    order=item['order'])
                logger.info(
                    f"[{tenant.schema_name}] Reordered FAQs for course {course_id}")
                return Response(
                    {"detail": "FAQs reordered successfully"}, status=status.HTTP_200_OK)
        except Exception as e:
            logger.error(
    f"[{tenant.schema_name}] Error reordering FAQs: {str(e)}",
     exc_info=True)
            return Response({"detail": "Error reordering FAQs"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class EnrollmentViewSet(TenantBaseView, viewsets.ViewSet):
    ...
    'user', 'course').filter(
        is_active=True).order_by('-enrolled_at')
            ...
            serializer = EnrollmentSerializer(
                page, many=True, context={'tenant': tenant})
            logger.info(f"[{tenant.schema_name}] Listed all enrollments")
            return paginator.get_paginated_response(serializer.data)
        except Exception as e:
            logger.error(
    f"[{tenant.schema_name}] Error fetching all enrollments: {str(e)}",
     exc_info=True)
            return Response({"detail": "Error fetching all enrollments"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def user_enrollments(self, request, user_id=None):
        ...
        try:
            ...
            if request.user.role != "admin" and user_id != str(
                request.user.id):
                logger.warning(
                    f"[{tenant.schema_name}] Non-admin user {request.user.id} attempted to access user {user_id} enrollments")
                return Response({"detail": "Unauthorized access"},
                                status=status.HTTP_403_FORBIDDEN)
            enrollments = Enrollment.objects.filter(user_id=user_id, is_active=True).select_related('course').prefetch_related(
                'course__resources', 'course__modules', 'course__modules__lessons', 'course__course_instructors__instructor__user'
    ...
    'name': ci.instructor.user.get_full_name(),
     'bio': ci.instructor.bio}
            ...
            logger.info(
                f"[{tenant.schema_name}] Retrieved enrollments for user {user_id}")
            return Response(result)
        except Exception as e:
            logger.error(
    f"[{tenant.schema_name}] Error fetching user enrollments: {str(e)}",
     exc_info=True)
            return Response({"detail": "Error fetching user enrollments"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    total_lessons) *
     100) if total_lessons else 0
            ...
            logger.error(
    f"[{tenant.schema_name}] Error fetching my courses: {str(e)}",
     exc_info=True)
            return Response({"detail": "Error fetching my courses"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def complete_lesson(self, request):
        ...
        obj.save()
        return Response(
            {'detail': 'Lesson marked as completed'}, status=status.HTTP_200_OK)
    ...
    def post(self, request):
        ...
        try:
            ...
            if not package:
                ...
                return Response(
                    {'detail': 'No SCORM package uploaded.'}, status=400)
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                # Find SCORM root (folder with index.html and
                # imsmanifest.xml)
                scorm_root = find_scorm_root(extract_dir)
                if scorm_root:
                    scorm_root_abs_path = os.path.join(
                        extract_dir, scorm_root)
                    manifest_path = os.path.join(
scorm_root_abs_path, "imsmanifest.xml")
                    launch_file = "index.html"
                    if os.path.exists(manifest_path):
                        title, launch_file, metadata = parse_scorm_manifest(
                            manifest_path)
                    else:
                ...
                return Response(
                    {'id': course.id, 'title': course.title}, status=201)
        except Exception as e:
            logger.error(
    f"Error in SCORMCourseCreateView: {str(e)}",
     exc_info=True)
...
def parse_scorm_manifest(manifest_path):
    ...
    title = root.find(
    './/ims:title',
    ns).text if root.find(
        './/ims:title',
         ns) is not None else 'Untitled SCORM'
    ...
    return title, launch_path, {'manifest': 'parsed'}
...
class SCORMxAPIViewSet(viewsets.ViewSet):
    ...
    def upload(self, request, pk=None):
        ...
        scorm_settings, _ = SCORMxAPISettings.objects.get_or_create(
            course=course)
...
scorm_root_abs_path, "imsmanifest.xml")
            ...
            scorm_settings.save()
        return Response(
            {'detail': 'SCORM package uploaded and extracted successfully.'})

    @action(detail=True, methods=['get'])
    def player(self, request, pk=None):
        tenant = request.tenant
        course = get_object_or_404(Course, pk=pk)
        scorm_settings = get_object_or_404(
SCORMxAPISettings, course=course, is_active=True)
        storage_service = get_storage_service()
        # scorm_settings.package should now be the HTML file, not the zip
        package_path = scorm_settings.package.name if hasattr(
scorm_settings.package, 'name') else str(
    scorm_settings.package)
        if package_path.endswith('.zip'):
            return Response(
                {'detail': 'SCORM package is not extracted.'}, status=400)
        public_url = storage_service.get_public_url(package_path)
        if not public_url:
            return Response(
                {'detail': 'No SCORM launch file found.'}, status=404)
    ...
    SCORMxAPISettings, course=course, is_active=True)
        ...
        if not public_url:
            return Response(
                {'detail': 'No SCORM index.html found.'}, status=404)
        return Response({'public_url': public_url})
...
class SCORMTrackingView(APIView):
    ...
    def post(self, request, course_id):
        ...
        logger.info(
            f"SCORM tracking updated for user {user.id} in course {course_id}")
...
def find_scorm_root(extract_dir):
    ...
    """
    for root, dirs, files in os.walk(extract_dir):
        if "imsmanifest.xml" in files and (
            "index.html" in files or "index_lms.html" in files):
