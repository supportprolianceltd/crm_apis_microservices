...
def api_gateway_view(request, path):
    ...
    try:
        ...
        if prefix == "notifications":
            ...
            # Ensure POST/PUT/PATCH/DELETE requests have trailing slashes
...
# def api_gateway_view(request, path):
#     """
#     Enhanced API Gateway view with Circuit Breaker, comprehensive error handling,
...
#             logger.warning(f"[{request_id}] Empty path received")
#             return JsonResponse({
#                 "error": "Invalid path",
#                 "request_id": request_id,
#                 "details": "API path cannot be empty"
#             }, status=400)
...
#                 "error": "Invalid path",
#                 "request_id": request_id,
#                 "details": "Invalid API path format"
#             }, status=400)
...
#                     "request_id": request_id,
#                     "details": "Notifications service not available"
#                 }, status=500)
...
#                 auth_routes = getattr(settings, 'AUTH_ROUTES', [])
#                 if prefix in auth_routes:
#                     base_url = settings.MICROSERVICE_URLS.get("auth_service")
#                     service_name = "auth_service"
#                 else:
#                     logger.error(f"[{request_id}] No route found for prefix: {prefix}")
#                     return JsonResponse({
#                         "error": f"Route not found",
#                         "request_id": request_id,
#                         "details": f"No microservice configured for /api/{prefix}/"
#                     }, status=404)
#             else:
#                 service_name = prefix
...
#                     "error": "Service configuration error",
#                     "request_id": request_id,
#                     "details": f"Base URL not configured for {service_name}"
#                 }, status=500)
...
#         if not circuit_breaker.is_service_available(service_name):
#             logger.warning(f"[{request_id}] Circuit breaker OPEN for service: {service_name}")
#             service_stats = circuit_breaker.get_service_stats(service_name)
#             return JsonResponse({
#                 "error": "Service temporarily unavailable",
#                 "request_id": request_id,
#                 "service": service_name,
#                 "circuit_breaker_state": service_stats['state'],
#                 "details": f"{service_name} is experiencing high failure rate. Please try again later.",
...
#         # Set proper Host header for the target service
#         if base_url:
#             try:
#                 host_without_scheme = base_url.split('//')[-1].split('/')[0]
#                 headers['Host'] = host_without_scheme
...
#         headers['X-Gateway-Request-ID'] = request_id
#         headers['X-Gateway-Service'] = service_name
#         headers['X-Forwarded-Timestamp'] = str(start_time)
#         headers['X-Forwarded-For'] = request.META.get('REMOTE_ADDR', '')
#         headers['X-Forwarded-Host'] = request.get_host()
#         headers['X-Forwarded-Proto'] = 'https' if request.is_secure() else 'http'
...
#         max_body_size = 50 * 1024 * 1024  # 50MB max body size
#         try:
#             body = request.body
...
#                     "request_id": request_id,
#                     "details": f"Request body exceeds size limit of {max_body_size} bytes",
...
#         # ======================== TIMEOUT CONFIGURATION ========================
#         timeout_config = getattr(settings, 'GATEWAY_TIMEOUTS', {}).get('default', 300)
        
#         # Adjust timeout based on endpoint type and service
#         if 'screen-resumes' in path or 'screen_resumes' in path:
#             timeout_config = 900  # 15 minutes for resume screening
#         elif 'parse-resume' in path or 'upload' in path or 'file' in path:
#             timeout_config = 600   # 10 minutes for file processing
#         elif service_name == 'notifications':
#             timeout_config = 30   # Shorter for notifications
#         elif service_name == 'auth_service':
#             timeout_config = 60   # 1 minute for auth
...
#                 'headers': headers,
#                 'params': request.GET,
#                 'timeout': (30, timeout_config),  # (connect_timeout, read_timeout)
#                 'verify': False,  # Disable SSL verification for internal services
#                 'stream': True,   # Stream large responses
...
#             logger.error(f"[{request_id}] Gateway timeout on /api/{path} after {request_time:.2f}s (timeout: {timeout_config}s)")
...
#         logger.info(
#             f"[{request_id}] Gateway forwarded: {method} /api/{path} -> {response.status_code} "
#             f"(Service: {service_name}, Time: {response_time:.2f}s)"
#         )

#         # Stream the response content to avoid memory issues
#         response_content = b''
#         max_response_size = 100 * 1024 * 1024  # 100MB max response size
        
#         try:
#             for chunk in response.iter_content(chunk_size=8192):
#                 response_content += chunk
#                 if len(response_content) > max_response_size:
#                     logger.error(f"[{request_id}] Response too large: {len(response_content)} bytes")
#                     return JsonResponse({
#                         "error": "Response too large",
#                         "request_id": request_id,
#                         "service": service_name,
#                         "details": f"Response from {service_name} exceeds size limit",
#                         "max_size": max_response_size
#                     }, status=502)
#         except Exception as e:
#             logger.error(f"[{request_id}] Error streaming response: {str(e)}")
#             return JsonResponse({
#                 "error": "Response processing error",
#                 "request_id": request_id,
#                 "service": service_name,
#                 "details": "Failed to process response from service"
#             }, status=502)
...
#         response_headers['X-Gateway-Request-ID'] = request_id
#         response_headers['X-Gateway-Service'] = service_name
#         response_headers['X-Gateway-Response-Time'] = f"{response_time:.3f}s"
#         response_headers['X-Gateway-Circuit-Breaker-State'] = circuit_breaker.get_service_stats(service_name)['state']
...
#     except Exception as e:
#         response_time = time.time() - start_time
#         logger.exception(f"[{request_id}] Unexpected gateway error on /api/{path} after {response_time:.2f}s: {str(e)}")
#         return JsonResponse({
#             "error": "Internal gateway error",
#             "request_id": request_id,
#             "details": str(e),
